...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsVisible(currentScroll < lastScrollY || currentScroll < 60);
    ...
    >
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <a href="#inicio" onClick={handleLogoClick} className="flex items-center space-x-3">
          ...
          <span className="text-xl md:text-2xl font-bold text-gray-800 tracking-tight">AutoKlic</span>
        </a>
          ...
          <nav className="flex space-x-6 text-sm font-semibold tracking-wide">
            {links.map((link) => (
              <a
                ...
                className="text-gray-700 hover:text-blue-600 hover:underline underline-offset-8 decoration-2 transition duration-300"
              >
                {link.name}
              </a>
            ))}
      ...
      >
        <nav className="flex flex-col p-4 space-y-4">
          ...
          <button
            ...
            className="bg-blue-600 text-white font-medium px-4 py-2 rounded-full mt-4 text-sm hover:bg-blue-700 transition"
          >
            Iniciar sesión
          </button>
        </nav>
